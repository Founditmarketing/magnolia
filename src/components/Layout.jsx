      ...
      <header className={`glass-nav ${sc ? "scrolled" : ""}`} style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, transition: "all 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: sc ? 76 : 96, transition: "height 0.4s" }}>
          ...
          <Link to="/" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 42, height: 42, background: "linear-gradient(135deg, var(--primary), #FF2A00)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(255, 90, 0, 0.4)", borderTop: "1px solid rgba(255, 255, 255, 0.3)" }}>
              <span style={{ color: "#fff", fontWeight: 900, fontSize: 20, fontFamily: "var(--font-display)" }}>M</span>
            </div>
            <div>
              <div style={{ color: "#fff", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 18, letterSpacing: 2, textTransform: "uppercase", lineHeight: 1.1 }}>Magnolia State</div>
              <div style={{ color: "var(--primary)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 11, letterSpacing: 4, textTransform: "uppercase" }}>Construction LLC</div>
            </div>
          </Link>
          ...
          <button className="mt" onClick={() => setOpen(!open)} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#fff", cursor: "pointer", padding: 10 }}>
            {open ? <I.X /> : <I.Menu />}
          </button>
        </div>
      </header>
      ...
      {open && <div className="glass" style={{ position: "fixed", inset: 0, zIndex: 999, paddingTop: 120, display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
        {links.map((l, i) => l.ch ? (
          <div key={i} style={{ textAlign: "center" }}>
            ...
            {l.ch.map((c, j) => <Link key={j} to={c.to} onClick={() => setOpen(false)} style={{ display: "block", color: "#fff", textDecoration: "none", padding: "8px 0", fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>{c.label}</Link>)}
          </div>
        ) : (
          <Link key={i} to={l.to} onClick={() => setOpen(false)} style={{ color: path === l.to ? "var(--primary)" : "#fff", textDecoration: "none", fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase" }}>{l.label}</Link>
        ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 56, marginBottom: 56 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
              <div style={{ width: 38, height: 38, background: "linear-gradient(135deg, var(--primary), #FF2A00)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#fff", fontWeight: 900, fontSize: 18, fontFamily: "var(--font-display)" }}>M</span></div>
              <div><div style={{ color: "#fff", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 15, letterSpacing: 2.5, textTransform: "uppercase", lineHeight: 1.1 }}>Magnolia State</div><div style={{ color: "var(--primary)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 10, letterSpacing: 3.5, textTransform: "uppercase" }}>Construction LLC</div></div>
            </div>
          ...
          <div>
            <h4 style={{ color: "#fff", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13, letterSpacing: 4, textTransform: "uppercase", marginBottom: 24 }}>Services</h4>
          ...
          <div>
            <h4 style={{ color: "#fff", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13, letterSpacing: 4, textTransform: "uppercase", marginBottom: 24 }}>Contact</h4>
